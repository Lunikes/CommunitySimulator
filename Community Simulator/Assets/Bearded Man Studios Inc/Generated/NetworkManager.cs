...
namespace BeardedManStudios.Forge.Networking.Unity
{
	public partial class NetworkManager : MonoBehaviour
	{
		...
		public GameObject[] ChatManngerNetworkObject = null;
		...
		private void CaptureObjects(NetworkObject obj)
		{
			...
			else if (obj is ChatManngerNetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					NetworkBehavior newObj = null;
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (ChatManngerNetworkObject.Length > 0 && ChatManngerNetworkObject[obj.CreateCode] != null)
						{
							var go = Instantiate(ChatManngerNetworkObject[obj.CreateCode]);
							newObj = go.GetComponent<ChatManngerBehavior>();
						}
					}
		...
		[Obsolete("Use InstantiateChatMannger instead, its shorter and easier to type out ;)")]
		public ChatManngerBehavior InstantiateChatManngerNetworkObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(ChatManngerNetworkObject[index]);
			var netBehavior = go.GetComponent<ChatManngerBehavior>();
			var obj = netBehavior.CreateNetworkObject(Networker, index);
			go.GetComponent<ChatManngerBehavior>().networkObject = (ChatManngerNetworkObject)obj;
		...
		/// <summary>
		/// Instantiate an instance of ChatMannger
		/// </summary>
		/// <returns>
		/// A local instance of ChatManngerBehavior
		/// </returns>
		/// <param name="index">The index of the ChatMannger prefab in the NetworkManager to Instantiate</param>
		/// <param name="position">Optional parameter which defines the position of the created GameObject</param>
		/// <param name="rotation">Optional parameter which defines the rotation of the created GameObject</param>
		/// <param name="sendTransform">Optional Parameter to send transform data to other connected clients on Instantiation</param>
		public ChatManngerBehavior InstantiateChatMannger(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(ChatManngerNetworkObject[index]);
			var netBehavior = go.GetComponent<ChatManngerBehavior>();
